...
public class Explosion : MonoBehaviour
{
	...
	IEnumerator Start()
	{
		...
		if (causeDamage)
		{
			...
			{
				float distance = Mathf.Max(Vector3.Distance(transform.position, col.transform.position), 0.1f);
				int damageAmount = Mathf.RoundToInt(damage * (1f / distance));
				if (damageAmount <= 0)
					continue;
				...
				// Fallback Easy Weapons legacy (SendMessage ChangeHealth)
